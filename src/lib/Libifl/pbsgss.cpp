...
int write_all(struct tcp_chan *chan, char *buf, unsigned int nbyte)
  {
  int ret;
  ret = tcp_puts(chan,buf,nbyte);
  if (ret < 0)
    return ret;
 ...
 * @return \c PBSGSS_OK
 */
int pbsgss_recv_token(struct tcp_chan *chan, int *flags, gss_buffer_t tok)
{
  int ret;
  ...
  errno=0;
  ret = tcp_gets(chan, reinterpret_cast<char *>(&char_flags), 1, pbs_tcp_timeout);
  if (ret < 0)
    {
  ...
  if (char_flags == 0)
    {
    lenbuf[0] = 0;
    ret = tcp_gets(chan, reinterpret_cast<char *>(&lenbuf[1]), 3, pbs_tcp_timeout);
    if (ret < 0)
      {
    ...
    {
    ret = tcp_gets(chan, reinterpret_cast<char *>(lenbuf), 4, pbs_tcp_timeout);
    if (ret < 0)
      {
      perror("Kerberos - pbsgss_recv_token/reading token length");
      return PBSGSS_ERR_READ;
  ...
  tok->value = static_cast<char *>(malloc(tok->length ? tok->length : 1));
  ...
  ret = tcp_gets(chan, reinterpret_cast<char *>(tok->value), tok->length, pbs_tcp_timeout);
  if (ret < 0)
    {
 ...
 */
int pbsgss_send_token(struct tcp_chan *chan, int flags, gss_buffer_t tok)
  {
  int ret;
  ...
  if (char_flags)
    {
    ret = write_all(chan, (char *)&char_flags, 1);
    if (ret != 1)
      {
  ...
  ret = write_all(chan, (char *)lenbuf, 4);
  if (ret < 0)
    {
  ...
  ret = write_all(chan, static_cast<char*>(tok->value), tok->length);
  if (ret < 0)
    {
...
int pbsgss_server_establish_context(struct tcp_chan *chan, gss_cred_id_t server_creds, gss_cred_id_t* client_creds, gss_ctx_id_t* context, gss_buffer_t client_name, OM_uint32* ret_flags)
  ...
  *context = GSS_C_NO_CONTEXT;
  ...
  do
    {
    if (pbsgss_recv_token(chan, &token_flags, &recv_tok) != PBSGSS_OK)
      {
    ...
    if (send_tok.length != 0)
      {
      if (pbsgss_send_token(chan, TOKEN_CONTEXT, &send_tok) != PBSGSS_OK)
...
const char *pbsgss_get_host_princname()
  ...
  if ((service_name = static_cast<char*>(malloc(strl))) == NULL)
    return NULL;
  ...
  princname = static_cast<char*>(malloc(buffer.length + 2));
  if (princname != NULL)
    {
    strncpy(princname,static_cast<char*>(buffer.value),buffer.length);
 ...
 * @return \c PBSGSS_OK on success
 */
int pbsgss_client_establish_context(struct tcp_chan *chan, char * service_name, gss_cred_id_t creds, gss_OID oid, OM_uint32 gss_flags, gss_ctx_id_t * gss_context, OM_uint32 *ret_flags)
  {
  ...
  *gss_context = GSS_C_NO_CONTEXT;
  ...
  do
    ...
    if (maj_stat == GSS_S_CONTINUE_NEEDED)
      {
      status = pbsgss_recv_token(chan, &token_flags, &recv_tok);
      if (status != 0)
        {
        if (gss_delete_sec_context(&min_stat, gss_context, GSS_C_NO_BUFFER) != GSS_S_COMPLETE)
          return PBSGSS_ERR_INTERNAL;
...
int pbsgss_client_authenticate(char *hostname, struct tcp_chan *chan, int delegate, int wrap)
  ...
  if (maj_stat != GSS_S_COMPLETE) // couldn't acquire credentials
    ...
    princname = static_cast<char*>(malloc(strlen("host@") + strlen(buf.nodename) + 1));
    if (princname == NULL)
      {
      return PBSGSS_ERR_INTERNAL;
      }
  ...
  service_name = static_cast<char*>(malloc(strl));
  ...
  oid = GSS_C_NULL_OID;
  retval = pbsgss_client_establish_context(chan, service_name, creds, oid, gss_flags, &gss_context, &ret_flags);
  free(service_name);
  ...
  if (wrap)
    {
    if (pbsgss_save_sec_context(&gss_context,ret_flags,chan) != PBSGSS_OK)
...
int pbsgss_save_sec_context(gss_ctx_id_t *context, OM_uint32 flags, struct tcp_chan *chan)
  ...
  if (flags & (GSS_C_INTEG_FLAG | GSS_C_CONF_FLAG))
    {
    if (DIS_tcp_set_gss(chan, *context, flags) != PBSGSS_OK)
      return PBSGSS_ERR_CONTEXT_SAVE;
