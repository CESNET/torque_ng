...
static krb5_error_code get_ticket(struct krb_holder *ticket, char *errbuf, size_t errbufsz)
  ...
  memset(&ticket->job_info->creds, 0, sizeof(ticket->job_info->creds));
  ticket->job_info->endtime = 0;
  ...
  krb5_timeofday(ticket->context, &now);
  ...
  memset(&creds, 0, sizeof(creds));
  opts.preauth_list = pa_types;
  opts.address_list = b ? &no_addrs : NULL;
  ...
  /* set target client */
  ticket->job_info->creds.client = ticket->job_info->client;

  /* set target server */
  ticket->job_info->creds.server = remote_tgs;
...
struct krb_holder *alloc_ticket()
  {
  struct krb_holder *ticket = static_cast<struct krb_holder*>(calloc(1,sizeof(struct krb_holder)));
  if (ticket == NULL)
    return NULL;
...
static int get_job_info_from_job(const job *pjob, const task *ptask, eexec_job_info job_info)
  ...
  if (ptask == NULL)
    ...
    ccname = static_cast<char*>(malloc(len+1));
    if (ccname != NULL)
...
static int get_job_info_from_principal(const char *principal, const char* jobid, eexec_job_info job_info)
  ...
  if ((buf = static_cast<char*>(malloc(bufsize))) == NULL)
    {
    free(princ);
    return PBSGSS_ERR_INTERNAL;
    }
