...
int req_gssauthenuser (struct batch_request *preq, struct tcp_chan *chan)
  {
  ...
  */
  ...
  if (now - lastcredstime > credlifetime)
    ...
    if (service_name == NULL)
      ...
      service_name = static_cast<char*>(malloc(sizeof(buf.nodename) + 6));
  ...
  if ((status = pbsgss_server_establish_context(chan, server_creds, NULL, &context, &client_name, &ret_flags)) < 0)
    {
  ...
  free(svr_conn[chan->sock].principal);

  svr_conn[chan->sock].principal = (char *)malloc(sizeof(char) * (1 + client_name.length));
  strncpy(svr_conn[chan->sock].principal,static_cast<char*>(client_name.value),client_name.length);
  svr_conn[chan->sock].principal[client_name.length] = '\0';
  memcpy(&(svr_conn[chan->sock].creds),&client_creds,sizeof(client_creds));
  svr_conn[chan->sock].cn_authen = PBS_NET_CONN_GSSAPIAUTH;
  ...
  pbsgss_save_sec_context(&context,ret_flags,chan); // TODO Handle error
  log_event(PBSEVENT_DEBUG,PBS_EVENTCLASS_SERVER,"req_gssauthenuser calling con_credent","");

  if ((status = gss_conn_credent(preq,chan->sock)) < 0)
    {
